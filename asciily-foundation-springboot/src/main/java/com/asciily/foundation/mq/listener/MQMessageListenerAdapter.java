package com.asciily.foundation.mq.listener;
...
import com.asciily.foundation.config.redis.serialize.JacksonSerializer;

/**
 * @author kakashi
 * @since 2018年10月22日
 */
public class MQMessageListenerAdapter implements InitializingBean, MessageListener {
	...
	private class MethodInvoker {
		...
		MethodInvoker(Object delegate, final String methodName) {
			...
			Assert.isTrue(lenient || !methods.isEmpty(), "Cannot find a suitable method named [" + c.getName() + "#" + methodName + "] - is the method public and has the proper arguments?");
		}
	...
	/**
	 * Create a new {@link MQMessageListenerAdapter} with default settings.
	 */
	...
	/**
	 * Create a new {@link MQMessageListenerAdapter} for the given delegate.
	 * 
	 * @param delegate
	 *            the delegate object
	 */
	public MQMessageListenerAdapter(Object delegate) {
		initDefaultStrategies();
	 ...
	 *            method to call when a message comes
	 * @see #getListenerMethodName
	 */
	public MQMessageListenerAdapter(Object delegate, String defaultListenerMethod) {
		this(delegate);
	 ...
	 * <p>
	 * If no explicit delegate object has been specified, listener methods are expected to present on this adapter instance, that is, on a
	 * custom subclass of this adapter, defining listener methods.
	 * 
	 * @param delegate
	 *            delegate object
	 */
	 ...
	 * <p>
	 * Delegates the message to the target listener method, with appropriate conversion of the message argument. In case of an exception,
	 * the {@link #handleListenerException(Throwable)} method will be invoked.
	 * 
	 * @param message
	 ...
	 *            the converted Redis request message, to be passed into the listener method as argument
